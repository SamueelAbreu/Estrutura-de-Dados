...
void lista::insereNaPosicao(int posicao, equipe elenco){
     if(posicao <= verificaTamanho() && posicao >=0){
          if (vazia()){
               insereNoInicio(elenco);
          }else if(posicao == 0){
               insereNoInicio(elenco);
          }else if(posicao == verificaTamanho()){
               insereNoFim(elenco);
          }else{
               noh* novo = new noh(elenco);
               noh* aux = primeiro;
        ...
        try {
            ...
            switch (comando) {
                ...
                case 'a': // remover
                    ...
                    break;                   
               case 't': //imprime o tamanho da lista
                    cout << minhaLista.verificaTamanho() << endl;                   
                    break;                
                case 'p': // limpar tudo
                    minhaLista.imprime();
                    break;
